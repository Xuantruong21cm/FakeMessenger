...
import org.app.common.BuildConfig
import org.app.common.R
import org.app.common.databinding.PopUpActionChooserBinding


class ActionSheetHelper private constructor(builder: Builder) {

  private var fragmentManager: FragmentManager = builder.fragmentManager
  private val dialogFragment = ActionChooserPopUp()
  ...
  // Actions
  private val actions: ArrayList<ActionSheet>?
  ...
  // Item CLick
  private var itemClick: ((action: ActionSheet) -> Unit)?
  ...
  class ActionChooserPopUp : DialogFragment() {
    ...
    private lateinit var actionChooserAdapter: ActionSheetAdapter
    ...
    // Actions
    private var actions: ArrayList<ActionSheet>? = null
    ...
    // Item CLick
    private var itemClick: ((action: ActionSheet) -> Unit)? = null
    private var nativeAdsView: NativeViewPreload? = null
    ...
    fun onViewCreated(view: View, savedInstanceState: Bundle?) {
      ...
      binding.actionPopupContent.setOnClickListener {
        dismiss()
      }
      setUpViews()
    }
    ...
    fun setUpViews() {
      ...
      Log.d("POPUP", "show ads")
      nativeAdsView?.showAds(binding.nativeAdsViewPopup)
    }
    ...
    private fun setUpActionsRecyclerView() {
      actionChooserAdapter = ActionSheetAdapter(
    ...
    private fun onActionSelected(action: ActionSheet) {
    ...
    private fun setupNativeAds() {
      nativeAdsView = NativeViewPreload(BaseApplication.getInstance(),
        listener = object : AdMobViewListener {
          override fun onAdViewStateChanged(state: AdState) {
            Log.i("ACTION SHEET", "onAdViewStateChanged: $state")
            if (state == AdState.LOADED) nativeAdsView?.showAds(binding.nativeAdsViewPopup)
          }
        })
    }
    ...
    fun setActions(actions: ArrayList<ActionSheet>?) {
    ...
    fun setItemClick(itemClick: ((action: ActionSheet) -> Unit)?) {
  ...
  class Builder(internal var fragmentManager: FragmentManager) {
    ...
    // Actions
    internal var actions: ArrayList<ActionSheet>? = null
    ...
    // Item CLick
    internal var itemClick: ((action: ActionSheet) -> Unit)? = null
    ...
    fun setActions(actions: ArrayList<ActionSheet>): Builder {
      this.actions = actions
      return this
    }
    ...
    fun setItemClick(itemClick: ((action: ActionSheet) -> Unit)): Builder {
      this.itemClick = itemClick
      return this
    }

    fun create(): ActionSheetHelper {
      return ActionSheetHelper(this)
    }
  }
}